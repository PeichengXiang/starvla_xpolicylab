...
def convert_spark(source,out,limit=None,keep=False):
    ...
    return convert(paths,out,'spark',limit,keep)
...
def convert_ego(source,out,limit=None,keep=False):
    ...
    return convert(paths,out,'ego',limit,keep)

def convert(paths,out,kind,limit,keep):
    if limit: paths=paths[:limit]
    if not paths: raise FileNotFoundError('no episodes found')
    out=Path(out)
    if out.exists() and not keep: shutil.rmtree(out)
        ...
        with h5py.File(p,'r') as f:
            if kind=='spark':
                ...
                frame_lists={
                    ...
                    'observation.images.cam_left_wrist':[black] * len(images),
                    'observation.images.cam_right_wrist':[black] * len(images),
                }
            ...
            video_meta={}
            for key,frames in frame_lists.items():
                vp=out/'videos'/key/'chunk-000'/f'file-{ei:03d}.mp4'; write_video(frames[:length],vp,fps)
    ...
    print(f'wrote {out}: episodes={len(erows)} frames={total} action_dim={dim}')
