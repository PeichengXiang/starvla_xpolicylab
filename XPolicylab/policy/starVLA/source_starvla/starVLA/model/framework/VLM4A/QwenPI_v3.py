    ...
    def forward(
        ...
        **kwargs,
        ...
        with torch.autocast("cuda", dtype=torch.float32):
            ...
            requires_exact_raw_action = (
                isinstance(xpolicylab_schema, Mapping)
                and xpolicylab_schema.get("version") == 1
                and xpolicylab_schema.get("robot_type")
            ...
            if requires_exact_raw_action and not (
                ...
                raise ValueError(
                    "Invalid XPolicy action contract: labels must be same-timestep "
                    "raw HDF5 actions and must not be derived from state."
                )
            if requires_exact_raw_action and actions.shape[1] != self.action_horizon:
                raise ValueError(
                    "Refusing to shift or truncate action labels: "
                    f"dataset chunk has {actions.shape[1]} timesteps but "
                    f"action_horizon={self.action_horizon}."
                )
            # Legacy non-XPolicy configs may intentionally include past action
            # context.  Preserve their historic tail selection; the raw-action
            # XPolicy contract above requires exact equality, so this slice is
            # an identity for EgoVLA/SParkArena.
