...
class _QWen3_VL_Interface(nn.Module):
    ...
    def __init__(self, config: Optional[dict] = None, **kwargs):
        ...
        attention_backends = {
            "root": getattr(model.config, "_attn_implementation", None),
            "language": getattr(model.model.language_model.config, "_attn_implementation", None),
            "vision": getattr(model.model.visual.config, "_attn_implementation", None),
        }
        print(f"[starVLA] Qwen3-VL attention backends: {attention_backends}")
        if attn_implementation == "flash_attention_2" and any(
            backend != "flash_attention_2" for backend in attention_backends.values()
        ):
            raise RuntimeError(
                "flash_attention_2 was requested but did not reach every Qwen3-VL "
                f"attention branch: {attention_backends}"
            )
