...
case "${env_cfg_type}" in
    ego_h1_inspire) yaml_file="${SCRIPT_DIR}/deploy_egovla.yml" ;;
    tianji_marvin_wuji) yaml_file="${SCRIPT_DIR}/deploy_sparkarena.yml" ;;
    *) yaml_file="${SCRIPT_DIR}/deploy.yml" ;;
esac

echo "[CLIENT] policy=${policy_name}, task=${task_name}, ckpt_name=${ckpt_name}, config=${yaml_file}, server=${policy_server_host}:${policy_server_port}"
