...
def validate_server_runtime_contract(
    ...
    *,
    ...
    expected_data_contract: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    if expected_data_contract is not None:
        actual_data_contract = metadata.get("training_data_contract")
        if not isinstance(actual_data_contract, dict):
            raise ValueError(
                "StarVLA server does not advertise the checkpoint's training_data_contract. "
                "Re-train or serve a checkpoint that records action provenance, cameras, and image size."
            )
        for key, expected_value in expected_data_contract.items():
            actual_value = actual_data_contract.get(key)
            if actual_value != expected_value:
                raise ValueError(
                    f"Incompatible StarVLA training data contract: {key}={actual_value!r}, "
                    f"expected {expected_value!r}."
                )
