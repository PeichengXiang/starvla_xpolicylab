...
starvla_default_unnorm_key="arx_x5"
case "${env_cfg_type}" in
    ego_h1_inspire)
        yaml_file="${SCRIPT_DIR}/deploy_egovla.yml"
        starvla_robot_type="xpolicylab_egovla"
    ...
    tianji_marvin_wuji)
        ...
        ;;
    *)
        # Preserve released RoboDojo and custom StarVLA evaluation paths.
...
starvla_unnorm_key="${STARVLA_UNNORM_KEY:-${starvla_default_unnorm_key}}"
...
echo "[SERVER] policy=${policy_name}, task=${task_name}, config=${yaml_file}, policy_server_port=${policy_server_port}, starvla_port=${starvla_server_port}"
...
if [[ -n "${starvla_robot_type:-}" ]]; then
    saved_data_mix="$(
        "${STARVLA_POLICY_PYTHON}" - "${checkpoint_run_dir}/config.full.yaml" <<'PY'
import sys
import yaml

with open(sys.argv[1], "r", encoding="utf-8") as stream:
    config = yaml.safe_load(stream) or {}
print(config.get("datasets", {}).get("vla_data", {}).get("data_mix", ""))
PY
    )"
    if [[ -z "${saved_data_mix}" ]]; then
        echo "[SERVER][ERROR] checkpoint config.full.yaml has no datasets.vla_data.data_mix" >&2
        exit 1
    fi
    if [[ -n "${STARVLA_XPOLICY_DATA_MIX:-}" && "${STARVLA_XPOLICY_DATA_MIX}" != "${saved_data_mix}" ]]; then
        echo "[SERVER][ERROR] STARVLA_XPOLICY_DATA_MIX=${STARVLA_XPOLICY_DATA_MIX} does not match checkpoint data_mix=${saved_data_mix}" >&2
        exit 2
    fi
    if [[ -n "${STARVLA_XPOLICY_ROBOT_TYPE:-}" && "${STARVLA_XPOLICY_ROBOT_TYPE}" != "${starvla_robot_type}" ]]; then
        echo "[SERVER][ERROR] STARVLA_XPOLICY_ROBOT_TYPE=${STARVLA_XPOLICY_ROBOT_TYPE} does not match env_cfg_type=${env_cfg_type}" >&2
        exit 2
    fi
    starvla_registry_env=(
