...
Checkpoints land in `checkpoints/<bench_name>-<ckpt_name>-<env_cfg_type>-<action_type>-<seed>/`; at eval time `ckpt_name` may be the short run name, the full run-directory name, or a path to a checkpoint directory. The trainer generates a per-run config from `qwen_pi_v3.yaml` under `.generated/`, requires the converted LeRobot dataset (checked via `meta/modality.json`), and infers the process count from a comma-separated `gpu_id`; trailing `extra_args` are forwarded to the upstream trainer. Overrides: `STARVLA_DATA_ROOT` (default `data/`), `STARVLA_DATA_MIX` (default `xpolicylab_runtime`), `STARVLA_XPOLICY_DATASET_NAME` (default `<bench_name>-<ckpt_name>-<env_cfg_type>-<action_type>`).
...
## This workspace: QwenPI-v3 benchmark runs

This checkout pins the upstream StarVLA source to `c00b8e18c1b5ba6228fd68a8f38acca8df4e938a`.
The training entry point uses `qwen_pi_v3.yaml`, Qwen3-VL-4B-Instruct, per-device batch 8,
80,000 steps, and checkpoint saves every 10,000 steps; eight GPUs therefore give global batch 64.
Set `WANDB_API_KEY` in the shell before training; the key is intentionally not stored in this tree.

`train.sh` and `process_data.sh` automatically use the repository-local
`.venv-starvla/bin/python` when it exists. Activate it manually when running
other StarVLA commands:

```bash
source .venv-starvla/bin/activate
```

The verified stack includes Torch 2.6.0+cu124, torchvision 0.21.0+cu124, Transformers 4.57.0, Accelerate 1.5.2, DeepSpeed 0.16.9, flash-attn 2.7.4.post1, and the HDF5/video/parquet tooling used by the converters.

- `SParkArena` / `tianji_marvin_wuji` / `joint`: 54D state/action and three real camera inputs.
- `EgoVLA` / `ego_h1_inspire` / `joint`: 38D bimanual state/action and one `cam_high` input.

For the SParkArena run, the training mixture contains only the dataset named by
`STARVLA_XPOLICY_DATASET_NAME` (default
`SParkArena-<ckpt_name>-tianji_marvin_wuji-joint`). Its episodes come from
`data/raw_sources/SParkArena/<task>/tianji_marvin_wuji/data/episode_*.hdf5`.
The three model images are `vision/cam_head/colors`,
`vision/cam_left_wrist/colors`, and `vision/cam_right_wrist/colors`, mapped to
`observation.images.cam_high`, `observation.images.cam_left_wrist`, and
`observation.images.cam_right_wrist`. The HDF5 JPEG bytes are decoded with
`cv2.imdecode` and no BGR/RGB swap. The converter resizes them to 224×224 and
uses the required RGB→BGR conversion only for the BGR input expected by
`cv2.VideoWriter.write`; the encoded MP4 is YUV420 and the StarVLA video
backend returns RGB frames matching the HDF5 decode. The 54D state is concatenated from
`state/left_arm_joint_states` (7), `state/left_ee_joint_states` (20),
`state/right_arm_joint_states` (7), and `state/right_ee_joint_states` (20).
The 54D absolute-joint action comes from the four corresponding `action/*`
keys in the same order, with no synthetic action target. Here `left_ee` and
`right_ee` denote 20D hand joint states, not Cartesian EE poses. The HDF5
`action/left_ee_poses` and `action/right_ee_poses` are 7D absolute pose fields
but are not read by this joint-action converter; it does not produce delta EE
targets. The task text comes
from HDF5 `instruction`. StarVLA applies q99 normalization to state and action
and trains on 16-step action windows. `conversion_manifest.json` in each
prepared dataset records its exact episode paths and source keys.
Datasets converted before this decode policy have channel-reversed training
videos; `train.sh` rejects them until they are rebuilt with `process_data.sh`.

The EgoVLA benchmark contract is single-view even though one raw task contains
additional camera arrays. Conversion always maps `observations/images/main` to
`cam_high` and supplies exact-zero RGB images for both wrist slots. It uses 38
selected dimensions from same-timestep `observations/qpos[t]` and raw HDF5
`action[t]`, and obtains the canonical task text from the task directory name.

The raw datasets may be mounted or linked from shared storage. `process_data.sh`
and `data_scripts/convert_dataset.py` materialize benchmark-specific LeRobot v3
datasets under `data/`. Training and EgoVLA inference both retain three model
slots and set `black_camera_names: [cam_left_wrist, cam_right_wrist]`; SParkArena
uses three real views.
